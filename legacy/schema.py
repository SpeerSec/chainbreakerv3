...
# Compared against raw byte slices from the keychain binary
SECURE_STORAGE_GROUP = b'ssgp'

# SecAuthenticationType
# Keys are bytes because struct.unpack('>4s', ...) returns bytes in Python 3
AUTH_TYPE = {
    b'ntlm': 'kSecAuthenticationTypeNTLM',
    b'msna': 'kSecAuthenticationTypeMSN',
    b'dpaa': 'kSecAuthenticationTypeDPA',
    b'rpaa': 'kSecAuthenticationTypeRPA',
    b'http': 'kSecAuthenticationTypeHTTPBasic',
    b'httd': 'kSecAuthenticationTypeHTTPDigest',
    b'form': 'kSecAuthenticationTypeHTMLForm',
    b'dflt': 'kSecAuthenticationTypeDefault',
    b'': 'kSecAuthenticationTypeAny',
    b'\x00\x00\x00\x00': 'kSecAuthenticationTypeAny'
}

# SecProtocolType
PROTOCOL_TYPE = {
    b'ftp ': 'kSecProtocolTypeFTP',
    b'ftpa': 'kSecProtocolTypeFTPAccount',
    b'http': 'kSecProtocolTypeHTTP',
    b'irc ': 'kSecProtocolTypeIRC',
    b'nntp': 'kSecProtocolTypeNNTP',
    b'pop3': 'kSecProtocolTypePOP3',
    b'smtp': 'kSecProtocolTypeSMTP',
    b'sox ': 'kSecProtocolTypeSOCKS',
    b'imap': 'kSecProtocolTypeIMAP',
    b'ldap': 'kSecProtocolTypeLDAP',
    b'atlk': 'kSecProtocolTypeAppleTalk',
    b'afp ': 'kSecProtocolTypeAFP',
    b'teln': 'kSecProtocolTypeTelnet',
    b'ssh ': 'kSecProtocolTypeSSH',
    b'ftps': 'kSecProtocolTypeFTPS',
    b'htps': 'kSecProtocolTypeHTTPS',
    b'htpx': 'kSecProtocolTypeHTTPProxy',
    b'htsx': 'kSecProtocolTypeHTTPSProxy',
    b'ftpx': 'kSecProtocolTypeFTPProxy',
    b'cifs': 'kSecProtocolTypeCIFS',
    b'smb ': 'kSecProtocolTypeSMB',
    b'rtsp': 'kSecProtocolTypeRTSP',
    b'rtsx': 'kSecProtocolTypeRTSPProxy',
    b'daap': 'kSecProtocolTypeDAAP',
    b'eppc': 'kSecProtocolTypeEPPC',
    b'ipp ': 'kSecProtocolTypeIPP',
    b'ntps': 'kSecProtocolTypeNNTPS',
    b'ldps': 'kSecProtocolTypeLDAPS',
    b'tels': 'kSecProtocolTypeTelnetS',
    b'imps': 'kSecProtocolTypeIMAPS',
    b'ircs': 'kSecProtocolTypeIRCS',
    b'pops': 'kSecProtocolTypePOP3S',
    b'cvsp': 'kSecProtocolTypeCVSpserver',
    b'svn ': 'kSecProtocolTypeCVSpserver',
    b'AdIM': 'kSecProtocolTypeAdiumMessenger',
    b'\x00\x00\x00\x00': 'kSecProtocolTypeAny'
}
...
class _APPL_DB_HEADER:
    STRUCT = Struct('> 4s i i i i')

    def __init__(self, buffer: bytes):
        (self.Signature, self.Version, self.HeaderSize, self.SchemaOffset,
         self.AuthOffset) = _APPL_DB_HEADER.STRUCT.unpack(buffer)


class _APPL_DB_SCHEMA:
    STRUCT = Struct('> i i')
...
class _TABLE_HEADER:
    ...
    def __init__(self, buffer: bytes):
        (self.TableSize, self.TableId, self.RecordCount, self.Records,
         self.IndexesOffset, self.FreeListHead,
...
class _DB_BLOB:
    ...
    def __init__(self, buffer: bytes):
        (self.CommonBlobBuffer, self.StartCryptoBlob, self.TotalLength,
         self.RandomSignature, self.Sequence, self.ParamsBuffer, self.Salt,
         self.IV, self.BlobSignature) = _DB_BLOB.STRUCT.unpack(buffer)
...
class _GENERIC_PW_HEADER:
    STRUCT = Struct('> I I I I I I I I I I I I I I I I I I I I I I')

    def __init__(self, buffer: bytes):
        (self.RecordSize, self.RecordNumber, self.Unknown2, self.Unknown3,
         self.SSGPArea, self.Unknown5, self.CreationDate, self.ModDate,
         self.Description, self.Comment, self.Creator, self.Type,
         self.ScriptCode, self.PrintName, self.Alias, self.Invisible,
         self.Negative, self.CustomIcon, self.Protected, self.Account,
         self.Service, self.Generic) = _GENERIC_PW_HEADER.STRUCT.unpack(buffer)


class _KEY_BLOB_REC_HEADER:
    STRUCT = Struct('> I I 124s ')

    def __init__(self, buffer: bytes):
        (self.RecordSize, self.RecordCount,
         self.Dummy) = _KEY_BLOB_REC_HEADER.STRUCT.unpack(buffer)
...
class _KEY_BLOB:
    ...
    def __init__(self, buffer: bytes):
        (self.CommonBlobBuffer, self.StartCryptoBlob, self.TotalLength,
         self.IV) = _KEY_BLOB.STRUCT.unpack(buffer)

        self.CommonBlob = _COMMON_BLOB(self.CommonBlobBuffer)


class _SSGP:
    """First 28 bytes are structured - remainder is the encrypted password."""
...
class _INTERNET_PW_HEADER:
    ...
    def __init__(self, buffer: bytes):
        (self.RecordSize, self.RecordNumber, self.Unknown2, self.Unknown3,
         ...
         self.ScriptCode, self.PrintName, self.Alias, self.Invisible,
         self.Negative, self.CustomIcon, self.Protected, self.Account,
         self.SecurityDomain, self.Server, self.Protocol, self.AuthType,
         self.Port, self.Path) = _INTERNET_PW_HEADER.STRUCT.unpack(buffer)


class _APPLE_SHARE_HEADER:
    STRUCT = Struct('> I I I I I I I I I I I I I I I I I I I I I I I I I I')
    ...
    def __init__(self, buffer: bytes):
        (self.RecordSize, self.RecordNumber, self.Unknown2, self.Unknown3,
         ...
         self.ScriptCode, self.PrintName, self.Alias, self.Invisible,
         self.Negative, self.CustomIcon, self.Protected, self.Account,
         self.Volume, self.Server, self.Protocol, self.AuthType,
         self.Address, self.Signature) = _APPLE_SHARE_HEADER.STRUCT.unpack(buffer)


class _X509_CERT_HEADER:
    STRUCT = Struct('> I I I I I I I I I I I I I I I')

    def __init__(self, buffer: bytes):
        (self.RecordSize, self.RecordNumber, self.Unknown1, self.Unknown2,
         self.CertSize, self.Unknown3, self.CertType, self.CertEncoding,
         self.PrintName, self.Alias, self.Subject, self.Issuer,
         self.SerialNumber, self.SubjectKeyIdentifier,
         self.PublicKeyHash) = _X509_CERT_HEADER.STRUCT.unpack(buffer)


class _SECKEY_HEADER:
    STRUCT = Struct('> I I I I I I I I I I I I I I I I I I I I I I I I I I I I I I I I I')

    def __init__(self, buffer: bytes):
        (self.RecordSize, self.RecordNumber, self.Unknown1, self.Unknown2,
         self.BlobSize, self.Unknown3, self.KeyClass, self.PrintName,
         self.Alias, self.Permanent, self.Private, self.Modifiable,
         self.Label, self.ApplicationTag, self.KeyCreator, self.KeyType,
         self.KeySizeInBits, self.EffectiveKeySize, self.StartDate,
         self.EndDate, self.Sensitive, self.AlwaysSensitive,
         self.Extractable, self.NeverExtractable, self.Encrypt,
         self.Decrypt, self.Derive, self.Sign, self.Verify,
         self.SignRecover, self.VerifyRecover, self.Wrap,
...
class _UNLOCK_BLOB:
    ...
    def __init__(self, buffer: bytes):
        (self.CommonBlobBuffer, self.MasterKey,
         self.BlobSignature) = _UNLOCK_BLOB.STRUCT.unpack(buffer)

        self.CommonBlob = _COMMON_BLOB(self.CommonBlobBuffer)


class _KEYCHAIN_TIME:
    STRUCT = Struct('>16s')
    STRPTIME_FORMAT = "%Y%m%d%H%M%SZ"

    def __init__(self, buffer: bytes):
        raw: bytes = _KEYCHAIN_TIME.STRUCT.unpack(buffer)[0]
        self.Value: str = raw.strip(b'\x00').decode('ascii', errors='replace')
        self.Time: datetime = datetime.strptime(self.Value, _KEYCHAIN_TIME.STRPTIME_FORMAT)
...
class _INT:
    ...
    def __init__(self, buffer: bytes):
        self.Value: int = _INT.STRUCT.unpack(buffer)[0]


class _FOUR_CHAR_CODE:
    """Returns the raw bytes from a 4-byte code field.
    Kept as bytes because PROTOCOL_TYPE and AUTH_TYPE dicts use bytes keys."""
    STRUCT = Struct('>4s')

    def __init__(self, buffer: bytes):
        self.Value: bytes = _FOUR_CHAR_CODE.STRUCT.unpack(buffer)[0]


class _LV:
    """Length-Value structure. Decodes the value to a UTF-8 string,
    stripping trailing null bytes."""

    def __init__(self, buffer: bytes, length: int):
        self.STRUCT = Struct(">" + str(length) + "s")
        raw: bytes = self.STRUCT.unpack(buffer)[0]
        self.Value: str = raw.strip(b'\x00').decode('utf-8', errors='replace')
